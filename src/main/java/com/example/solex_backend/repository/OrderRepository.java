...
public interface OrderRepository extends JpaRepository<Order, Long> {
    @Query("SELECT o FROM Order o WHERE o.user = :user AND o.id < :cursor AND (:status IS NULL OR o.status = :status) ORDER BY o.id DESC")
    ...
    @Query("SELECT o FROM Order o WHERE o.restaurant.id = :restaurantId AND o.id < :cursor AND (:status IS NULL OR o.status = :status) ORDER BY o.id DESC")
