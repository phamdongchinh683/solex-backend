...
public class PaymentService {
    ...
    public PaymentIntentResponse initiatePayment(User user, CreatePaymentRequest request, String clientIp) {
        ...
        PaymentStrategy strategy = strategies.stream()
                ...
                .findFirst()
                .orElseThrow(
                        () -> new BusinessException("Payment method not supported: " + request.method()));
    ...
    public PaymentResponse getPaymentById(Long id, User user) {
        ...
        if (!payment.getOrder().getUser().getId().equals(user.getId())) {
            throw new BusinessException("You do not have permission to view this payment transaction");
        }
